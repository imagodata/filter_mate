...
def build_combined_expression(
    ...
    if normalized_old == normalized_new:
        logger.debug("[PostgreSQL] New filter identical to existing - replacing instead of combining")
        return new_where_clause
...
def execute_filter_action_postgresql(
    ...
    has_expensive_spatial_expression(sql_subset_string)
    ...
    try:
        ...
        if where_clause:
            ...
            if fallback_to_materialized_fn:
                logger.info("[PostgreSQL] Falling back to materialized view approach")
...
def execute_filter_action_postgresql_materialized(
    ...
    if custom:
        ...
        if param_buffer_expression and parse_where_clauses_fn:
            ...
            logger.warning("[PostgreSQL] Custom buffer requested but param_buffer_expression is None, using simple view")
