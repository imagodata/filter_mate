...
class ParallelFilterExecutor:
    ...
    def filter_layers_parallel(
        ...
        if shared_database_files:
            ...
            logger.warning(
                "⚠️ Multiple layers share the same SQLite database - using SEQUENTIAL execution. "
        ...
        if has_ogr_layers:
            logger.warning(
                "⚠️ OGR layers detected - using SEQUENTIAL execution for thread safety. "
                "Parallel execution of OGR operations causes access violations because "
                "QGIS layer objects are not thread-safe. "
                f"Providers: {provider_types}"
            )
            return self._filter_sequential(layers, filter_func, progress_callback, cancel_check)
        ...
        if is_geometric:
            logger.warning(
                "⚠️ Geometric filtering detected - using SEQUENTIAL execution for thread safety. "
                "Geometric operations use selectByLocation which is not thread-safe."
            )
            return self._filter_sequential(layers, filter_func, progress_callback, cancel_check)
    ...
    def _filter_sequential(
        ...
        # Log entry to sequential filter
        ...
        # Log layer names to be processed
        layer_names = []
        for l, lp in layers:
            try:
                layer_names.append(l.name() if hasattr(l, 'name') else 'unknown')
            except Exception:  # Avoid bare except clause
        ...
        # Check cancel_check status at start
        if cancel_check:
        ...
        # Track SQLite database file paths for inter-layer delay
        ...
        for i, (layer, layer_props) in enumerate(layers):
            # Check for cancellation at the start of each layer processing
            ...
            # DISABLED cancel_check during distant layer filtering
            ...
            # Add inter-layer delay for same SQLite database
