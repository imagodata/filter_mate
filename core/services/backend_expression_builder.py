...
class BackendExpressionBuilder:
    ...
    def _get_source_filter_for_postgresql(self) -> Optional[str]:
        ...
        if use_task_features:
            ...
            if has_buffer_expression and self.source_layer:
                logger.info("   📋 Buffer expression active without selection - using ALL source features")
                ...
                if source_filter:
                    ...
                    logger.warning("   ⚠️ Failed to build filter from all source features")
            else:
                logger.warning("   ⚠️ NO SOURCE FILTER! Source layer has no subsetString and no selection")
                logger.warning("   → EXISTS will match ALL source features!")
    ...
    def _build_filter_from_task_features(self, task_features: List) -> Optional[str]:
        ...
        if not pk_field:
            logger.warning("⚠️ PostgreSQL EXISTS: Could not determine primary key field for source layer")
            return None

        fids = extract_feature_ids(task_features, pk_field, self.source_layer)
        if not fids:
            logger.warning("⚠️ PostgreSQL EXISTS: Could not extract feature IDs from task_features")
            return None
    ...
    def _build_filter_with_mv(
        ...
        try:
            ...
            if mv_ref:
                ...
                logger.info("   ℹ️ Check ERROR logs above for details (connection, table info, SQL)")
    ...
    def _build_filter_from_visible_features(self) -> Optional[str]:
        ...
        Returns:
            str or None: The source filter expression
        """
        logger.info("🔄 PostgreSQL EXISTS: Generating filter from currently visible source features")
        logger.info("   → Source layer has filtered subset but it contains unadaptable patterns")
        logger.info("   → Fetching visible feature IDs to create new source_filter")

        try:
            pk_field = sfb_get_primary_key_field(self.source_layer)
            if not pk_field:
                logger.warning("   ⚠️ Could not determine primary key field for source layer")
                return None

            visible_fids = get_visible_feature_ids(self.source_layer, pk_field)
            if not visible_fids:
                logger.warning("   ⚠️ No visible features found in source layer!")
                return None
    ...
    def _build_filter_from_all_source_features(self) -> Optional[str]:
        ...
        if current_filter:
            logger.info("🔄 PostgreSQL EXISTS: Generating filter from source layer (filtered state)")
            logger.info(f"   → Respecting existing filter: {current_filter[:80]}...")
        else:
            logger.info("🔄 PostgreSQL EXISTS: Generating filter from ALL source features")
            logger.info("   → No existing filter - using all features")
    ...
    def _get_source_wkt_info(self) -> Tuple[Optional[str], Optional[str], Optional[int]]:
        ...
        if source_wkt:
            ...
            logger.debug(
                "PostgreSQL: spatialite_source_geom not available (expected for EXISTS mode with source_filter)"
            )
                    ...
                    if optimization_plan:
                        ...
                        if optimization_plan.final_simplify_tolerance and optimization_plan.final_simplify_tolerance > 0:
                            ...
                            if not filtering_params.get("has_simplify_tolerance", False):
                                ...
                                logger.info("🎯 AUTO-OPTIMIZATION: Buffer simplification enabled")
    ...
    def build(self, backend, layer_props: Dict, source_geom) -> Optional[str]:
        ...
        if backend.get_backend_name() == 'PostgreSQL':
            ...
            logger.debug("Geometric filtering: Non-PostgreSQL backend, source_filter=None")
        ...
        logger.info("📐 BackendExpressionBuilder.build - Buffer being passed to backend:")
        ...
        if expression == "__USE_OGR_FALLBACK__":
            logger.warning("Backend returned USE_OGR_FALLBACK sentinel - forcing OGR fallback")
            logger.info("  → GeometryCollection conversion failed, RTTOPO MakeValid would error")
            return None

        if not expression:
            logger.warning("No expression generated by backend")
            return None
