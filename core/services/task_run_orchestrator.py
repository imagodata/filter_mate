...
class TaskRunOrchestrator:
    ...
    def run(self, context: TaskRunContext) -> TaskRunResult:
        ...
        try:
            ...
            if result is False:
                ...
                logger.error("     Check Python console for detailed error messages.")
                logger.error("     Common causes: no features selected, empty filter expression, database connection issue.")
                return TaskRunResult(
                    success=False,
    ...
    def _get_or_generate_session_id(self, task_params: Dict[str, Any]) -> str:
        ...
        session_id = hashlib.md5(f"{time.time()}".encode(), usedforsecurity=False).hexdigest()[:8]
