...
# Import constants
# Constants - previously from modules.constants
...
from ...infrastructure.utils.type_utils import can_cast, return_typed_value
...
class LayersManagementEngineTask(QgsTask):
        ...
        if layer_provider_type == PROVIDER_POSTGRES:
            ...
            if not POSTGRESQL_AVAILABLE:
                logger.info(
                    ...
                    "   Basic filtering works, but for MUCH faster performance:\n"
                    "   → Install: pip install psycopg2-binary\n"
                    "   → Enables materialized views (10-100x faster for large datasets)"
                )
    ...
    def create_spatial_index_for_postgresql_layer(self, layer, layer_props):
        ...
        if "infos" not in layer_props:
            logger.warning("layer_props missing 'infos' dictionary, skipping spatial index creation")
            return False
