...
class UIConfig:
    ...
    _PROFILE_CONFIGS: Dict[str, Dict[str, Any]] = {
        ...
        #
        # IMPORTANT: Qt auto-scales pixel values by devicePixelRatio at render
        # time, so HIDPI must NOT pre-amplify sizes — that would double-scale
        # ("logical px × profile multiplier × DPR multiplier"), producing huge
        # buttons on 4K screens. HIDPI keeps NORMAL-equivalent values for
        # buttons/widget_keys/dockwidget; only combobox/input get a modest bump
        # (28px vs NORMAL's 20px) for legibility on dense filter panels, since
        # 20px renders too cramped at high DPI even after Qt's DPR scaling.
        DisplayProfile.HIDPI.value: {
            ...
            # combobox/input bumped to 28px (vs NORMAL=20) for HDPI legibility.
            # Both must stay equal — the QSS substitutes them in the same
            # selector group used for QComboBox/QLineEdit/QSpinBox.
            'combobox': {
                'height': 28,
                'min_height': 28,
                'max_height': 28,
