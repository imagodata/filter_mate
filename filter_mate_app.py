...
class FilterMateApp:
    ...
    def __init__(self, plugin_dir):
        ...
        import time
        import hashlib
        self.session_id = hashlib.md5(f"{time.time()}_{os.getpid()}_{id(self)}".encode(), usedforsecurity=False).hexdigest()[:8]
        ...
        if USE_APP_INITIALIZER and self._app_initializer is not None:
            ...
            try:
                ...
                if success:
                    ...
                    logger.error("AppInitializer returned False - falling back to legacy initialization")
            except Exception as e:
    ...
    def _safe_layer_operation(self, layer, properties, operation):
        ...
        def deferred_operation():
            ...
            if not is_valid_layer(fresh_layer):
                logger.debug("_safe_layer_operation: fresh layer is invalid, skipping")
                return
    ...
    def _legacy_dispatch_task(self, task_name: str, data=None):
        ...
        if task_parameters is None:
            ...
            return
        logger.info("✓ Task parameters built successfully")
    ...
    def _save_current_layer_before_filter(self):
        ...
        if self._current_layer_before_filter:
            try:
                ...
                logger.info("v4.1: 💾 Saved current_layer before filtering")
    ...
    def init_filterMate_db(self):
        ...
        if success:
            ...
            if hasattr(self, 'favorites_manager') and self.db_file_path and self.project_uuid:
                logger.debug("Configuring FavoritesService with database:")
