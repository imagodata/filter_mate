...
- **PostgreSQL feature list deferred while the canvas renders**: right after a project opened, the same 12 ms request blocked the main thread for 17.7 s (the provider connection pool is busy with the render jobs of the 18 PostgreSQL layers). The population now waits for the canvas to refresh, or 8 s at most, instead of freezing the panel. Idle layer change on a PostgreSQL layer: 134 ms.
