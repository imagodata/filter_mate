...
- Same scenario, second cause: `TaskOrchestrator` kept its own `add_layers` counter, incremented on every dispatch and decremented by nobody, so after the first registration of a session every later `add_layers` (project switch, layer added to the project) was queued for ever ("Queueing add_layers - 1 task(s) in progress"). `FilterMateApp.manage_task` is the single `add_layers` queue now.
- Same scenario, third cause: the project-change handlers reset the filtering-tab layer combo with `setLayer(None)` followed by `clear()`; `QComboBox.clear()` empties the `QgsMapLayerProxyModel` for good, so the combo showed 0 layers after a switch. `setLayer(None)` alone now, enforced by an AST test.
- Same scenario, fourth cause: `layerStore().layersAdded.disconnect()` (and `layersWillBeRemoved`, `allLayersRemoved`, `PROJECT.fileNameChanged`) without a slot removed every receiver of those QGIS-owned signals, QgsProject's own forwarding included, so from the first project switch on the layer combo kept the previous project's entries (18 through a 37-layer project and an empty one). The plugin now disconnects its own slots only; an AST test forbids a bare `disconnect()` on QGIS-owned signals. Verified in QGIS 4.2 with the panel open across 31.qgz → bdd.qgz → 31.qgz → empty → 31.qgz: combo and registered layers follow the project every time.
...
- **Type to find a layer**: the current-layer combo box of the filtering tab is editable with a "contains", case-insensitive completer (nothing is inserted into the model; the text goes back to the current layer's name when the edit loses focus). Verified in QGIS 4.2: choosing a completion changes the current layer of the panel.
...
- **Layer change on PostgreSQL: the feature list was rebuilt twice, the first time with an empty expression** (`⏱ picker_populate: 17353 ms … via setDisplayExpression < setLayer < _refresh_feature_pickers_for_field_change`, then 442 ms via the reload step, measured in QGIS 4.2 on `construction_lineaire`). `current_layer_changed` clears the expression widgets before setting the layer's expressions; every `fieldChanged` reached `_refresh_feature_pickers_for_field_change`, which rebuilt the pickers. While a layer change is in progress the pickers are left to the reload step. The `⏱ picker_populate` line now names its caller chain, and the synchronization step no longer populates the list with a provisional expression either.
- **PostgreSQL feature list: no server-side ORDER BY on the limited request.** Measured in QGIS 4.2 (LIMIT 1001, no geometry): `ORDER BY "cleabs"` 13.6 s and `ORDER BY` the primary key 11.0 s on troncon_de_route (370 690 rows), 16.8 s / 12.6 s on batiment (681 654 rows), against 24 ms without ORDER BY: the order is not pushed to the server, the whole table is fetched and sorted client-side before the limit applies, and every layer change on a PostgreSQL layer paid it (18.5 s on construction_lineaire during a project load). The list now takes the first rows PostgreSQL returns, sorted by display value client-side; the text filter still searches the whole layer server-side and "Select All" loads everything. GeoPackage keeps the server order (0.6 s on 1.17 M rows).
- **PostgreSQL feature list deferred while the canvas renders**: right after a project opened, the same 12 ms request blocked the main thread for 17.7 s (the provider connection pool is busy with the render jobs of the 18 PostgreSQL layers). The population now waits for the canvas to refresh (fallback 15 s, re-armed up to three times while the canvas still renders) instead of freezing the panel. Idle layer change on a PostgreSQL layer: 134 ms.
- Project switch: the layers are registered once. The `layersAdded` debounce of the project read already registered them before the 2.5 s reinitialization timer fired, and the deferred `add_layers` registered them a second time (second `LayersManagementEngineTask` and UI rebuild per switch); it now registers only the layers still missing, or lowers the loading flag.
- The Filter / Unfilter button shows pressed while its task runs (from the canvas freeze to the thaw); it stays enabled, a second click still cancels and relaunches.
- Empty PostgreSQL targets are reported once per filter task (one WARNING and one QGIS message-log line listing the layers) instead of one warning per layer.
...
- The "corrupted subset" cleaner run before each target filter only accepted subsets starting with `EXISTS (`; with the envelope prefilter in front of the EXISTS every PostgreSQL target was judged corrupted, and the queued empty subset was applied on the main thread after the worker had set the new filter, leaving the targets unfiltered. A well-formed EXISTS block anywhere in the subset is now accepted.
- **PostgreSQL EXISTS filters scanned the whole target layer** (measured after the `ST_DWithin` change: 456 s for 16 layers, 322 s on `zone_de_vegetation`, 35 s on `batiment`, for 300 road segments with a 20 m buffer): PostgreSQL ran the correlated subquery for every target row. The EXISTS is now preceded by `"target"."geom" && (SELECT ST_SetSRID(ST_Expand(ST_Extent(__source.geom)::geometry, d), MAX(ST_SRID(__source.geom))) FROM source AS __source WHERE <selection>)`: the envelope of the selected source rows, grown by the buffer, computed once and served by the GiST index of the target - the counterpart of the GeoPackage R-tree prefilter. Skipped for `ST_Disjoint`, for a reprojected target geometry and when the whole source table is the selection.
- **PostgreSQL source-selection materialized view never survived**: its indexes were created on the source column names (`geometrie`, `fid`) instead of the view's aliases (`geom`, `pk`); the failed statement aborted the transaction, the view was rolled back with it ("MV creation reported success but MV not found") and every large selection fell back to the inline `IN (...)` clause. Index names fixed, optional statements run in a savepoint, and the view is now committed so QGIS's own connection can see it; a failure rolls the connection back instead of leaving it in "current transaction is aborted" state.
- **Reload on project switch with auto-activation disabled** (the default): the `projectRead` / `newProjectCreated` / `cleared` signals were only connected when `APP.AUTO_ACTIVATE` was true, so an open panel never ran its project-change reinitialization and only followed the `layersAdded` / `allLayersRemoved` events of the layer store, keeping the previous project's layers, source layer and favorites. The signals now stay connected; the setting only governs auto-opening the panel. The pending `layersAdded` batch is cancelled before the full reinitialization so the layers are registered once.
- OGR backend: the buffered source layer is computed once per task and reused by every target layer (it was rebuilt for each of the 36 targets), it honours the task's buffer segments and end-cap style (hard-coded to 8 segments, round), and the primary-key values of the selected features are fetched with one request instead of one `getFeature()` per selected id.
- Applying the queued subsets: expressions up to 1 MB are applied inside the frozen-canvas loop (the limit was 100 KB, so every ~150 KB buffered Spatialite expression went through the deferred path with a repaint and a full count per layer: 4.6 s of post-filter UI work for 36 layers). The deferred path no longer counts features.
- Feature iterator retries (`unable to open database file`, `database is locked`) are logged at INFO with their wait time, next to the `⏱` lines.
...
- Auto-zoom after a spatial cascade (intersects, within, touches, ...) uses the source layer's filtered extent, grown by the buffer distance on projected CRSs, instead of the union of every filtered layer: computing 17 PostgreSQL target extents cost 2.2 s (feature scan) to 9.8 s (server-side `ST_Extent`, tried and reverted) on the main thread. Attribute-only cascades and the `disjoint` / `contains` predicates keep the union.
- Applying the queued subsets after a filter: the canvas is frozen for the whole loop and refreshed once, the fixed 0.2-0.5 s pauses between layers of the same SQLite/GeoPackage file are removed, and a `unable to open database file` / `database is locked` error on a shared file is retried once after 0.3 s. A 12-layer GeoPackage cascade no longer pays up to 5.5 s of blind pauses on the main thread.
- OGR spatial selection: the target layer is copied into a repaired memory layer only when a validity scan finds an invalid geometry (the copy of up to 50 000 features per target ran on every filter); `⏱ ogr_safe_source` / `ogr_target_validity_scan` / `ogr_safe_target` / `ogr_selectbylocation` are logged when they exceed 250 ms.
- Worker-side cascade over layers of the same SQLite/GeoPackage file: the fixed 0.2-0.5 s pause before each layer is removed (measured: 17.5 s of a 19.5 s filter on a 37-layer GeoPackage project); a genuine lock error is retried once after 0.3 s.
- No auto-zoom after an unfilter or a reset (the union extent is the whole dataset; the render it launched made the next filter wait 84 s on a 37-layer GeoPackage project). The in-progress render is cancelled before the queued subsets are applied, and `⏱ apply_subset_layer` is logged for any layer slower than 250 ms.
- Worker cascade: no `featureCount()` and no `triggerRepaint()` per target layer any more (a COUNT query per PostgreSQL layer, a misleading unfiltered total on Spatialite/OGR whose subsets are only queued, and a canvas repaint request per layer during the filter). The zero-feature warning uses a LIMIT 1 probe on PostgreSQL.
- `⏱ project_open` now closes when the layer registration task completes; it never fired before.
- Worker cascade over the target layers: no `featureCount()` before and after each target any more. On PostgreSQL, where the subset is applied in the worker, each call was a real `SELECT count(*)` with the spatial predicate (the provider cache is invalidated by every subset change); measured at ~0.45 s per target on 17 BD TOPO tables. New timing lines `⏱ layer_filter` (targets slower than 200 ms) and `⏱ cascade_filter`.
- The map canvas is frozen for the whole filter/unfilter/reset task and refreshed once at the end (a watchdog thaws it after 10 minutes; the subset-application loop no longer thaws a canvas it did not freeze). Every `setSubsetString` issued by the worker asked for a repaint, so a 17-target PostgreSQL cascade could start up to 17 renders of the project while it was still running.
- GeoPackage targets: the Spatialite expression now starts with the R-tree candidate clause GDAL uses for its own spatial filter (`ROWID IN (SELECT id FROM rtree_<table>_<geom> WHERE minx <= … AND maxx >= … AND miny <= … AND maxy >= …)`), added only when the R-tree table exists (read-only lookup, cached 120 s) and the predicate is not Disjoint. SQLite evaluated `ST_Intersects(ST_PointOnSurface(geom), <43 KB polygon>)` row by row over the whole table: filtering 37 layers by the commune of Toulouse took 371 s, 164 s of them on the 1.2 M-row `batiment` table.
- Applying the queued subsets: no `reload()` after a successful `setSubsetString` on PostgreSQL and OGR layers (the provider already signals the change; on GeoPackage the reload reopened the file and discarded the count QGIS had just computed, so `featureCount()` ran the filter a second time), and no `featureCount()` on PostgreSQL layers on the main thread (a real query with the spatial predicate; the count is free on OGR). `⏱ apply_subset_layer` now details the set and count phases.
- PostgreSQL targets with the centroid option: the predicate `ST_Intersects(ST_PointOnSurface("t"."geom"), source)` cannot use the GiST index of `geom`, so every target row was scanned (16 s on the 1.2 M-row `batiment` table for a five-commune selection). The expression now reads `("t"."geom" && source AND ST_Intersects(ST_PointOnSurface("t"."geom"), source))`: the bounding-box test brings the index back and never changes the result. Not applied to ST_Disjoint nor to a reprojected target geometry. `safe_set_subset_string` logs `⏱ set_subset` with its phases (type check, casting, `setSubsetString`) when a call takes 250 ms or more.
- A filter launched with the source layer only (no target layer checked) failed with `ZeroDivisionError` in the progress computation; the action now completes.
